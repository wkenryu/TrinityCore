...
#pragma pack(pop)
    ...
    public:
        ...
        bool StartNetwork(boost::asio::io_service& service, std::string const& bindIp) { return StartNetwork(service, bindIp, 1119); }
        ...
        void AddLoginTicket(std::string const& id, std::unique_ptr<Session::AccountInfo> accountInfo);
        std::unique_ptr<Session::AccountInfo> VerifyLoginTicket(std::string const& id);
    ...
    private:
        ...
        {
            std::string Id;
            std::unique_ptr<Session::AccountInfo> Account;
